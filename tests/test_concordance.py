...
def test_alpha_concordance_w(get_data):
    ...
    _, y_alpha = AlphaConcordance(0).fit_transform(X, y, w_0)
...
def test_alpha_concordance_y(get_data):
    ...
    _, y_alpha = AlphaConcordance(1).fit_transform(X, y, w_0)
    assert y == pytest.approx(y_alpha, 0.1)

    _, y_alpha = AlphaConcordance(1, True).fit_transform(X, y, w_0)
...
def test_gamma_squared_concodance_y(get_data):
    ...
    _, y_gamma = GammaSquaredConcordance(0).fit_transform(X, y, w_0)
    w_gamma = np.linalg.pinv(X) @ y_gamma

    np.testing.assert_almost_equal(y, y_gamma, 3)
    assert w_gt == pytest.approx(w_gamma, 0.1)

    _, y_gamma = GammaSquaredConcordance(0, True).fit_transform(X, y, w_0)
...
def test_gamma_squared_concodance_w(get_data):
    ...
    _, y_gamma = GammaSquaredConcordance(1e9).fit_transform(X, y, w_0)
    w_gamma = np.linalg.pinv(X) @ y_gamma
